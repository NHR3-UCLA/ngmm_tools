...
RunINLA <- function(df_flatfile, df_cellinfo, df_cellmat, out_fname, out_dir, res_name='res',
  ...
  #station data
  data_sta_all <- df_flatfile[,c('ssn','Vs30','x_3','staX','staY')]
  out_unq   <- UniqueIdxInv(df_flatfile[,'ssn'])
  sta_idx   <- out_unq$idx
  sta_inv   <- out_unq$inv
  data_sta  <- data_sta_all[sta_idx,]
  X_sta     <- data_sta[,c(4,5)] #station coordinates
  X_sta_all <- data_sta_all[,c(4,5)]
  ...
  prior_fixed <- list(mean.intercept = 0, prec.intercept = 5,
                      mean = (list(intcp=0.0, R=-0.005, x2=c_2_erg, x3=c_3_erg, default=0)),
                      prec = (list(intcp=5.0, R=10000,  x2=25.0,    x3=25.0,    default=0.01)))
  ...
  #spde eq constant prior
  spde_eq_const <- inla.spde2.pcmatern(mesh = mesh, alpha = alpha, # Mesh and smoothness parameter
                                       prior.range = c(100, 0.95), # P(range < 100) = 0.95
                                       prior.sigma = c(.30, 0.1))  # P(sigma > 0.30) = 0.10
  #spde sta constant prior
  spde_sta_const <- inla.spde2.pcmatern(mesh = mesh, alpha = alpha, # Mesh and smoothness parameter
                                        prior.range = c(100, 0.95), # P(range < 100) = 0.95
                                        prior.sigma = c(.40, 0.1))  # P(sigma > 0.40) = 0.10
  #spde gp term prior
  spde_eq_gs     <- inla.spde2.pcmatern(mesh = mesh, alpha = alpha, # Mesh and smoothness parameter
                                        prior.range = c(100, 0.95), # P(range < 100) = 0.95
                                        prior.sigma = c(.30, 0.1))  # P(sigma > 0.30) = 0.10
  #spde Vs30 term prior
  spde_sta_vs30  <- inla.spde2.pcmatern(mesh = mesh, alpha = alpha, # Mesh and smoothness parameter
                                        prior.range = c(100, 0.95), # P(range < 100) = 0.95
                                        prior.sigma = c(.40, 0.1))  # P(sigma > 0.40) = 0.10
  
  A_eq_const    <- inla.spde.make.A(mesh, loc = as.matrix(X_eq_all))
  idx.eq_const  <- inla.spde.make.index("idx.eq_const",spde_eq_const$n.spde)
  A_sta_const   <- inla.spde.make.A(mesh, loc = as.matrix(X_sta_all))
  idx.sta_const <- inla.spde.make.index("idx.sta_const",spde_sta_const$n.spde)
  A_eq_gs       <- inla.spde.make.A(mesh, loc = as.matrix(X_eq_all), weights = x_2)
  idx.eq_gs     <- inla.spde.make.index("idx.eq_gs",spde_eq_gs$n.spde)
  A_sta_vs30    <- inla.spde.make.A(mesh, loc = as.matrix(X_sta_all), weights = x_3)
  ...
  form_inla_spatial <- y ~ 0 + intcp + x2 + x3 + R +
                           ...
                           f(idx.eq_const, model = spde_eq_const) + f(idx.sta_const, model = spde_sta_const) +
                           f(idx.eq_gs,    model = spde_eq_gs)    + f(idx.sta_vs30,  model = spde_sta_vs30) +
                           f(idx_cell, model = "z", Z = RC_sparse, hyper=prior_omega_ca)
  
  
  #build stack
  stk_inla_spatial <- inla.stack(data = list(y = y_data),
                                 A = list(A_eq_const, A_sta_const, A_eq_gs, A_sta_vs30, 1),
                                 effects = list(idx.eq_const = idx.eq_const, idx.sta_const = idx.sta_const, 
                                                idx.eq_gs = idx.eq_gs, idx.sta_vs30 = idx.sta_vs30,
                                                df_inla_covar),
  ...
  if (! all(is.na(post_ell_3s$y_int))){  
    ...
    hyp_posterior$ell_3s_pdf   <- approx(post_ell_3s$y_int,   post_ell_3s$y,   hyp_posterior$quant)$y
  } else {
    hyp_posterior$ell_1as       <- NaN
    hyp_posterior$ell_1as_pdf   <- NaN
  }
  ...
  if (! all(is.na(post_omega_3s$y_int))){  
    ...
    hyp_posterior$omega_3s_pdf <- approx(post_omega_3s$y_int, post_omega_3s$y, hyp_posterior$quant)$y
  } else {
    hyp_posterior$omega_3s     <- NaN
    hyp_posterior$omega_3s_pdf <- NaN
  }
  ...
  pl_dc_2p_sd_map <- pl_dc_2p_sd_map + geom_path(data=map_ca, aes(x=X,y=Y), color='black') + geom_path(data=map_nv, aes(x=X,y=Y), color='black') +
                    ...
                    labs(x="X (km)", y="Y (km)") + theme(axis.title = element_text(size=20), axis.text.y = element_text(size=20), axis.text.x = element_text(size=20))
  #dc_3s map mean
  pl_dc_3s_mu_map <- ggplot() + theme_bw()
  pl_dc_3s_mu_map <- plot_field(coeff_3s$mean, mesh, xrange=c(-200,800), yrange=c(3400,4750), pl=pl_dc_3s_mu_map)
  pl_dc_3s_mu_map <- pl_dc_3s_mu_map + geom_path(data=map_ca, aes(x=X,y=Y), color='black') + geom_path(data=map_nv, aes(x=X,y=Y), color='black') +
                     geom_point(data=X_sta, aes(x=staX,y=staY), color=I("black"), size=0.2) +
                     labs(x="X (km)", y="Y (km)") + theme(axis.title = element_text(size=20), axis.text.y = element_text(size=20), axis.text.x = element_text(size=20))
  #dc_3s map sigma
  pl_dc_3s_sd_map <- ggplot() + theme_bw()
  pl_dc_3s_sd_map <- plot_field(coeff_3s$sd, mesh, xrange=c(-200,800), yrange=c(3400,4750), pl=pl_dc_3s_sd_map)
  pl_dc_3s_sd_map <- pl_dc_3s_sd_map + geom_path(data=map_ca, aes(x=X,y=Y), color='black') + geom_path(data=map_nv, aes(x=X,y=Y), color='black') +
                     geom_point(data=X_sta, aes(x=staX,y=staY), color=I("black"), size=0.2) +
                     labs(x="X (km)", y="Y (km)") + theme(axis.title = element_text(size=20), axis.text.y = element_text(size=20), axis.text.x = element_text(size=20))
  ...
  pl_omega_3s_post <- ggplot(post_omega_3s, aes(x,y)) + theme_bw() + geom_line() + 
                      geom_vline(xintercept = hyp_param['omega_3s','mean'], colour = "red") +
                      labs(x = 'omega_3s', y = 'posterior', title='Posterior omega_3s') + 
                      theme(plot.title=element_text(size=20), axis.title=element_text(size=20), axis.text.y=element_text(size=20), axis.text.x=element_text(size=20))
  #ell_1e
  pl_ell_1e_post <- ggplot(post_ell_1e, aes(x,y)) + theme_bw() + geom_line() + 
                    geom_vline(xintercept = hyp_param['ell_1e','mean'], colour = "red") +
                    labs(x = 'omega_1e', y = 'posterior', title='Posterior ell_1e') + 
                    theme(plot.title=element_text(size=20), axis.title=element_text(size=20), axis.text.y=element_text(size=20), axis.text.x=element_text(size=20))
  #ell_1as
  ...
  pl_ell_3s_post <- ggplot(post_ell_3s, aes(x,y)) + theme_bw() + geom_line() + 
                    geom_vline(xintercept = hyp_param['ell_3s','mean'], colour = "red") +
                    labs(x = 'ell_3s', y = 'posterior', title='Posterior ell_3s') + 
                    theme(plot.title=element_text(size=20), axis.title=element_text(size=20), axis.text.y=element_text(size=20), axis.text.x=element_text(size=20))
  #omega_ca
